...
def extract_haplotype_ids(bam: IO, output: IO, chroms: Iterable[str], include_unphased: bool):
    print("Reading bam file")
    if len(chroms) == 1 and chroms[0] == "all":
        chroms = None
    ps_collection = extract_read_haplotype_assignment(bam, chroms=chroms, return_unphased=include_unphased)
        ...
        for ps_id, ps in ps_collection.phased.items():
            for chrom_reads in ps.read_mappings.values():
                for read in chrom_reads:
                    out_f.write(out_line.format(read_name=read.read_name, hp=read.haplotype, ps=ps_id))
