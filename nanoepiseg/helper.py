...
class plotting:
    ...
    def plot_segment_profile(matrix, samples, segment, segment_p):
        ...
        for s in ['s3','s2','s1']:
            ...
            plt.scatter(x,y, c=color, marker='|',s=15, cmap='RdYlGn_r', vmin=-2.5, vmax=2.5)
    #        if s == 's3':
    #            plt.colorbar()
...
class IntervalFilter(RegionFilter) : 
    def __init__(self, start, end):
        ...
        self.end = end
    def filter(allmet):
        return allmet.loc[allmet['start'].map(lambda x: x >= self.start and x <self.end)]
...
class QuantileFilter(RegionFilter) : 
    ...
    def filter(self, allmet):
        ...
        return allmet.loc[allmet['start'].map(lambda x: x >= start and x < end)]

class MultiIntervalFilter(RegionFilter):
    def __init__(self, ranges):
        self.ranges = ranges
    def filter(self, allmet):
        passed = allmet['start'].map(lambda x:any([x >= r[0] and x < r[1] for r in self.ranges]))
        return allmet.loc[passed]

class medulloblastoma_project:

    def __init__(self, samples=['s1','s2','s3']):
        self.samples = samples

    def load_pickled_metcalls(self, metcall_dir : str, chrom : str, 
            region_filter : RegionFilter = RegionFilter(), filter_bad_reads:bool=False):
        self.chrom = chrom
        sample_met = dict()
        rn_sample_dict = dict()
        for s in self.samples:
            sample_file = os.path.join(metcall_dir, '%s_%s_met_cpg.pkl'%(s,chrom))
            sample_met[s] = pd.read_pickle(sample_file, compression='gzip')
            rn_sample_dict = {**rn_sample_dict, **{r:s for r in list(set(sample_met[s]['read_name']))}}
        ...
        if filter_bad_reads:
            while True:
                ...
                allmet = allmet[allmet['start'].map(lambda x: not x  in bad_locs)]
                ...
                allmet = allmet[allmet['read_name'].map(lambda x: not x in bad_reads)]
        ...
        read_end = allmet[['read_name', 'start']].groupby('read_name').max().start
    ...
    def build_sparse_met_matrix(self):
        ...
        unique_sites_dict = {unique_sites[i]:i for i in range(len(unique_sites))}
    ...
    def compute_distance_matrix(self, l_met_matrix=None, weigh_by_site_heterogeniety=False, min_cov=30,
        ...
        overlap_size = overlap_size - sp.diags(overlap_size.diagonal(), dtype='uint32')
        ...
        met_matrix_met.data = activation_function(met_matrix_met.data, weight=1-total_met_rate)
        met_matrix_unmet = met_matrix_unmet.multiply(l_met_matrix)
        met_matrix_unmet.data = activation_function(-met_matrix_unmet.data, weight=total_met_rate)
        
        if weigh_by_site_heterogeniety:
            site_heterogeniety = 1-(np.abs(met_matrix_met.sum(axis=0) - \
                met_matrix_unmet.sum(axis=0)))/(met_matrix_met.sum(axis=0) + met_matrix_unmet.sum(axis=0))
        ...
        disagreement = (met_matrix_unmet * met_matrix_met.T + met_matrix_met * met_matrix_unmet.T).multiply(A_full)
